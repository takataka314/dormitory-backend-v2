...
async function initDefaultStaff() {
  const defaultStaff = {
    name: "marusitsu",
    email: "keiteki326sikkou@gmail.com",
    pin: "0000",
  };
  ...
  try {
    const result = await client.query(
      "SELECT id FROM users WHERE name = $1 AND is_staff = 1",
      [defaultStaff.name]
    );
    ...
    if (result.rowCount === 0) {
      await client.query(
        ...
        VALUES ($1, $2, $3, 1)
      `,
        [defaultStaff.name, defaultStaff.email, hashPin(defaultStaff.pin)]
      );
      console.log(`✔ 初期スタッフを登録しました: ${defaultStaff.name}`);
    } else {
      console.log("✔ スタッフは既に登録済みです");
    }
  } finally {
    client.release();
  }
...
app.listen(PORT, async () => {
  try {
    await initDb();
    console.log("🚀 Server running & DB ready");
  } catch (e) {
